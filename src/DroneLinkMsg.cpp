...
void DroneLinkMsg::printPayload(DRONE_LINK_PAYLOAD *payload, uint8_t paramTypeLength)
{
  ...
  case DRONE_LINK_MSG_TYPE_CHAR:
    Serial.print(F("c"));
    break;
  case DRONE_LINK_MSG_TYPE_NAME:
    Serial.print(F("N"));
    break;
  case DRONE_LINK_MSG_TYPE_NAMEQUERY:
    Serial.print(F("NQ"));
    break;
  case DRONE_LINK_MSG_TYPE_QUERY:
    Serial.print(F("Q"));
    break;
  }
  if (ty != DRONE_LINK_MSG_TYPE_QUERY)
  {
    Serial.print('[');
    Serial.print(len);
    if ((paramTypeLength & DRONE_LINK_MSG_WRITABLE) > 0)
      Serial.print(",W");
    Serial.print(']');
    Serial.print('=');
  }
  ...
  for (uint8_t i = 0; i < numValues; i++)
  {
    ...
    {
    case DRONE_LINK_MSG_TYPE_UINT8_T:
      Serial.print(payload->uint8[i]);
      break;
    case DRONE_LINK_MSG_TYPE_UINT32_T:
      Serial.print(payload->uint32[i]);
      break;
    case DRONE_LINK_MSG_TYPE_FLOAT:
      Serial.print(payload->f[i]);
      break;
    case DRONE_LINK_MSG_TYPE_CHAR:
      Serial.print(char(payload->c[i]));
      break;
    case DRONE_LINK_MSG_TYPE_ADDR:
      printAddress(&payload->addr[i]);
      break;
...
void DroneLinkMsg::printPayload(DRONE_LINK_PAYLOAD *payload, uint8_t paramTypeLength, AsyncResponseStream *response)
{
  ...
  for (uint8_t i = 0; i < numValues; i++)
  {
    ...
    {
    case DRONE_LINK_MSG_TYPE_UINT8_T:
      response->print(payload->uint8[i]);
      break;
    case DRONE_LINK_MSG_TYPE_UINT32_T:
      response->print(payload->uint32[i]);
      break;
    case DRONE_LINK_MSG_TYPE_FLOAT:
      response->print(payload->f[i], 8);
      break;
    case DRONE_LINK_MSG_TYPE_CHAR:
      response->print(char(payload->c[i]));
      break;
    case DRONE_LINK_MSG_TYPE_ADDR:
      printAddress(&payload->addr[i], response);
      break;
    }
    if (ty != DRONE_LINK_MSG_TYPE_CHAR)
      response->print(' ');
...
void DroneLinkMsg::printFloatAsJson(float f, AsyncResponseStream *response)
{
  if (isnan(f))
  {
    response->print("null");
  ...
  {
    response->print("4294967040.0");
  }
  else if (fabs(f) > 4294967040.0)
  {
    response->print("4294967040.0");
  }
  else
...
void DroneLinkMsg::printPayloadAsJson(DRONE_LINK_PAYLOAD *payload, uint8_t paramTypeLength, AsyncResponseStream *response)
{

  uint8_t ty = (paramTypeLength >> 4) & 0x7;
  uint8_t len = (paramTypeLength & 0xF) + 1;
  response->printf("\"type\":%u", ty);
  if (ty != DRONE_LINK_MSG_TYPE_QUERY)
  {
    response->printf(",\"writeable\":%u", ((paramTypeLength & DRONE_LINK_MSG_WRITABLE) > 0) ? 1 : 0);
    response->printf(",\"size\":%u", len);
  }
  ...
  for (uint8_t i = 0; i < numValues; i++)
  {
    if (i > 0 && (ty != DRONE_LINK_MSG_TYPE_CHAR))
      response->print(",");
    switch (ty)
    {
    case DRONE_LINK_MSG_TYPE_UINT8_T:
    ...
    case DRONE_LINK_MSG_TYPE_UINT32_T:
      ...
      break;
    case DRONE_LINK_MSG_TYPE_FLOAT:
      printFloatAsJson(payload->f[i], response);
      break;
    case DRONE_LINK_MSG_TYPE_CHAR:
      response->print(char(payload->c[i]));
      break;
