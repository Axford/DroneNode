...
HMC5883LModule::HMC5883LModule(uint8_t id, DroneSystem* ds):
  I2CBaseModule ( id, ds )
 {
   ...
   _params[HMC5883L_PARAM_SAMPLES_E].paramPriority = setDroneLinkMsgPriorityParam(DRONE_LINK_MSG_PRIORITY_LOW, HMC5883L_PARAM_SAMPLES);
   _params[HMC5883L_PARAM_SAMPLES_E].name = FPSTR(STRING_SAMPLES);
   _params[HMC5883L_PARAM_SAMPLES_E].nameLen = sizeof(STRING_SAMPLES);
   _params[HMC5883L_PARAM_SAMPLES_E].paramTypeLength = _mgmtMsg.packParamLength(false, DRONE_LINK_MSG_TYPE_UINT32_T, 16);
   _params[HMC5883L_PARAM_SAMPLES_E].data.uint32[0] = 0;
   _params[HMC5883L_PARAM_SAMPLES_E].data.uint32[1] = 0;
   _params[HMC5883L_PARAM_SAMPLES_E].data.uint32[2] = 0;
   _params[HMC5883L_PARAM_SAMPLES_E].data.uint32[3] = 0;
...
void HMC5883LModule::loop() {
  ...
  */
  ...
  if (headingDegrees > -20 && headingDegrees < 20) {
    updateQuadrant(0, _params[HMC5883L_PARAM_VECTOR_E].data.f[1]);
  }

  // y-
  if (headingDegrees > 160 && headingDegrees < 200) {
    updateQuadrant(2, _params[HMC5883L_PARAM_VECTOR_E].data.f[1]);
  }

  // x+
  if (headingDegrees > 70 && headingDegrees < 110) {
    updateQuadrant(3, _params[HMC5883L_PARAM_VECTOR_E].data.f[0]);
  }

  // x-
  if (headingDegrees > 250 && headingDegrees < 290) {
    updateQuadrant(1, _params[HMC5883L_PARAM_VECTOR_E].data.f[0]);
  }
...
void HMC5883LModule::updateQuadrant(uint8_t quadrant, float v) {
  float avgWindow = 20;

  // start applying noise rejection after sufficient samples received
  if (_params[HMC5883L_PARAM_SAMPLES_E].data.uint32[quadrant] > 100) {
    float ratio = v / _params[HMC5883L_PARAM_LIMITS_E].data.f[quadrant];
    if (ratio < 0.8 || ratio > 1.2) return;
  }

  _params[HMC5883L_PARAM_LIMITS_E].data.f[quadrant] =
    (_params[HMC5883L_PARAM_LIMITS_E].data.f[quadrant] * (avgWindow-1) +  v
  ) / avgWindow;

  _params[HMC5883L_PARAM_SAMPLES_E].data.uint32[quadrant]++;
}
