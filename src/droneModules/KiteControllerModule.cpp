  ...
  DroneModule ( id, ds )
{
  ...
  sub = &_subs[KITE_CONTROLLER_SUB_TARGET_E];
  sub->addrParam = KITE_CONTROLLER_SUB_TARGET_ADDR;
  sub->param.paramPriority = setDroneLinkMsgPriorityParam(DRONE_LINK_MSG_PRIORITY_LOW, KITE_CONTROLLER_SUB_TARGET);
  setParamName(FPSTR(STRING_TARGET), &sub->param);
  sub->param.data.f[0] = 0;
  sub->param.data.f[1] = 0;

  sub = &_subs[KITE_CONTROLLER_SUB_HEADING_E];
  sub->addrParam = KITE_CONTROLLER_SUB_HEADING_ADDR;
  sub->param.paramPriority = setDroneLinkMsgPriorityParam(DRONE_LINK_MSG_PRIORITY_LOW, KITE_CONTROLLER_SUB_HEADING);
  setParamName(FPSTR(STRING_HEADING), &sub->param);

  sub = &_subs[KITE_CONTROLLER_SUB_DISTANCE_E];
  sub->addrParam = KITE_CONTROLLER_SUB_DISTANCE_ADDR;
  sub->param.paramPriority = setDroneLinkMsgPriorityParam(DRONE_LINK_MSG_PRIORITY_LOW, KITE_CONTROLLER_SUB_DISTANCE);
  setParamName(FPSTR(STRING_DISTANCE), &sub->param);

  _subs[KITE_CONTROLLER_SUB_TARGET_E].enabled = false;
  _subs[KITE_CONTROLLER_SUB_DISTANCE_E].enabled = false;
  ...
  param = &_params[KITE_CONTROLLER_PARAM_THRESHOLD_E];
  param->paramPriority = setDroneLinkMsgPriorityParam(DRONE_LINK_MSG_PRIORITY_LOW, KITE_CONTROLLER_PARAM_THRESHOLD);
...
void KiteControllerModule::loop() {
  ...
  if (_lastMode != _params[KITE_CONTROLLER_PARAM_MODE_E].data.uint8[0]) {
    if (_params[KITE_CONTROLLER_PARAM_MODE_E].data.uint8[0] == KITE_CONTROLLER_MODE_MANUAL) {
      // zero distance to stop motors when entering manual
      _subs[KITE_CONTROLLER_SUB_DISTANCE_E].param.data.f[0] = 0;

    }
    _lastMode = _params[KITE_CONTROLLER_PARAM_MODE_E].data.uint8[0];
  }
  ...
  if (_params[KITE_CONTROLLER_PARAM_MODE_E].data.uint8[0] == KITE_CONTROLLER_MODE_MANUAL) {
    ...
    _subs[KITE_CONTROLLER_SUB_TARGET_E].enabled = true;
    _subs[KITE_CONTROLLER_SUB_DISTANCE_E].enabled = true;
  }
  ...
  // get distance to go
  float d = _subs[KITE_CONTROLLER_SUB_DISTANCE_E].param.data.f[0];
  float threshold = _params[KITE_CONTROLLER_PARAM_THRESHOLD_E].data.f[0];
  ...
  */
  ...
  /*
  if (!_subs[KITE_CONTROLLER_SUB_HEADING_E].received ||
      !_subs[KITE_CONTROLLER_SUB_TARGET_E].received) return;
  */

  // local shortcuts
  float h = _subs[KITE_CONTROLLER_SUB_HEADING_E].param.data.f[0];
  float t = _subs[KITE_CONTROLLER_SUB_TARGET_E].param.data.f[0];
  ...
  // clamp i error
  if (_params[KITE_CONTROLLER_PARAM_PID_E].data.f[1] > 0) {
    if (fabs(_iError) > 100 / _params[KITE_CONTROLLER_PARAM_PID_E].data.f[1]) {
      _iError = (_iError > 0 ? 100 : -100) / _params[KITE_CONTROLLER_PARAM_PID_E].data.f[1];
    }
  }

  // apply PID cooefficients
  turnRate =
    err * _params[KITE_CONTROLLER_PARAM_PID_E].data.f[0] +
    _iError * _params[KITE_CONTROLLER_PARAM_PID_E].data.f[1] +
    _dError * _params[KITE_CONTROLLER_PARAM_PID_E].data.f[2];
  ...
  */
  ...
  // local shortcuts
  float smin = _params[KITE_CONTROLLER_PARAM_LIMITS_E].data.f[0];
  float smax = _params[KITE_CONTROLLER_PARAM_LIMITS_E].data.f[1];
  ...
  if (_subs[KITE_CONTROLLER_SUB_DISTANCE_E].param.data.f[0] < 1) {
    speed = 0;
    turnRate = 0;
  ...
  x += _params[KITE_CONTROLLER_PARAM_TRIM_E].data.f[0];
