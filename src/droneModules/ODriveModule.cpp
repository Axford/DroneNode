...
float ODriveModule::readFloatFromSerial() {
  _bufLen = 0;
  _port->flush();
  delay(5); // to wait for oDrive to respond
...
void ODriveModule::loop()
{
  ...
  f[0] = (readFloatFromSerial() + _params[ODRIVE_PARAM_SAMPLES_E].data.uint8[0] * _params[ODRIVE_PARAM_TORQUE_E].data.f[0]) / (_params[ODRIVE_PARAM_SAMPLES_E].data.uint8[0] + 1);

  _port->write("r axis1.motor.current_control.Iq_measured\n");
  f[1] = (readFloatFromSerial() + _params[ODRIVE_PARAM_SAMPLES_E].data.uint8[0] * _params[ODRIVE_PARAM_TORQUE_E].data.f[1]) / (_params[ODRIVE_PARAM_SAMPLES_E].data.uint8[0] + 1);

  updateAndPublishParam(&_params[ODRIVE_PARAM_TORQUE_E], (uint8_t*)&f, sizeof(f));
}