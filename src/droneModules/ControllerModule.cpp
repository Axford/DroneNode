...
ControllerModule::ControllerModule(uint8_t id, DroneModuleManager* dmm, DroneLinkManager* dlm):
  I2CBaseModule ( id, dmm, dlm )
 {
   ...
   // init sub labels
   for (uint8_t i=0; i<CONTROLLER_NUM_LABELS; i++) {
     _subLabels[i][0] = '0' + i;
     _subLabels[i][1] = '\0';
     _labelState[i] = CONTROLLER_LABEL_NOT_NEEDED;
   }
   ...
   sub = &_subs[CONTROLLER_SUB_SUB1_E];
   sub->addrParam = CONTROLLER_SUB_SUB1_ADDR;
   sub->param.param = CONTROLLER_SUB_SUB1;
   setParamName(FPSTR(DRONE_STR_SUB1), &sub->param);

   sub = &_subs[CONTROLLER_SUB_SUB2_E];
   sub->addrParam = CONTROLLER_SUB_SUB2_ADDR;
   sub->param.param = CONTROLLER_SUB_SUB2;
   setParamName(FPSTR(DRONE_STR_SUB2), &sub->param);

   sub = &_subs[CONTROLLER_SUB_SUB3_E];
   sub->addrParam = CONTROLLER_SUB_SUB3_ADDR;
   sub->param.param = CONTROLLER_SUB_SUB3;
   setParamName(FPSTR(DRONE_STR_SUB3), &sub->param);

   sub = &_subs[CONTROLLER_SUB_SUB4_E];
   sub->addrParam = CONTROLLER_SUB_SUB4_ADDR;
   sub->param.param = CONTROLLER_SUB_SUB4;
...
void ControllerModule::onOTAProgress(float progress) {
  // show a progress bar
  DroneWire::selectChannel(_bus);
  ...
  for (uint8_t i=0; i<_numSubs; i++) {
    if (msg->sameAddress(&_subs[i].addr)) {
      if (msg->type() == DRONE_LINK_MSG_TYPE_NAME) {
        ...
        _labelState[i] = CONTROLLER_LABEL_RECEIVED;
...
void ControllerModule::loop() {
  ...
  if (loopTime > _lastDiscovery + 5000) {
    ...
    for (uint8_t i=0; i<CONTROLLER_NUM_LABELS; i++) {
      if (_labelState[i] == CONTROLLER_LABEL_REQUESTED) _labelState[i] = CONTROLLER_LABEL_NEEDED;
    }
  }
  ...
  for (uint8_t i=0; i<CONTROLLER_NUM_LABELS; i++) {
    ...
    if (_subs[i].received) {
      // trigger discovery
      if (_labelState[i] == CONTROLLER_LABEL_NOT_NEEDED) _labelState[i] = CONTROLLER_LABEL_NEEDED;

      if (_labelState[i] == CONTROLLER_LABEL_REQUESTED) labelRequested = true;

      if (_labelState[i] == CONTROLLER_LABEL_NEEDED && labelRequested == false) {
        // send a namequery message
        _labelState[i] = CONTROLLER_LABEL_REQUESTED;
        labelRequested = true;
