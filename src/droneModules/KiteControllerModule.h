/*

@type          KiteController
@inherits      Drone
@category      Logic
@description   Generate left and right motor speeds from target course, current heading and distance to target

@guide >>>

<<<

@config >>>
[ KiteController = 8 ]
  name = KiteController
  mode = 1
  trim = 0
  PID = 0.02, 0, 0
  limits = 0.2,1
  threshold = 15
  $target = @>Nav.adjTarget
  $heading = @>50.8
  $distance = @>Nav.distance
  publish = moode, trim, PID, limits, threshold, target
  publish = heading, distance, left, right
<<<

*/
#ifndef KITE_CONTROLLER_MODULE_H
#define KITE_CONTROLLER_MODULE_H

#include "../DroneModule.h"

//pubs
// @pub 8;f;1;r;left;Left motor speed output in range -1..1
#define KITE_CONTROLLER_PARAM_LEFT          8
#define KITE_CONTROLLER_PARAM_LEFT_E        0

// @pub 9;f;1;r;right;Right motor speed output in range -1..1
#define KITE_CONTROLLER_PARAM_RIGHT         9
#define KITE_CONTROLLER_PARAM_RIGHT_E       1

// @pub 16;u8;1;w;mode;1=Automated (responds to speed and turnRate subs) or 0=Manual (ignores subs, but speed and turnRate values can be directly written to)
#define KITE_CONTROLLER_PARAM_MODE          10
#define KITE_CONTROLLER_PARAM_MODE_E        2

// @pub 14;f;1;w;trim;Applied to turnRate value to correct left/right imbalance in motor power
#define KITE_CONTROLLER_PARAM_TRIM          11
#define KITE_CONTROLLER_PARAM_TRIM_E        3

// @pub 12;f;3;w;PID;PID settings for turnRate control
#define KITE_CONTROLLER_PARAM_PID           12
#define KITE_CONTROLLER_PARAM_PID_E         4
 
// @pub 12;f;2;w;limits;Min and max output speed range
#define KITE_CONTROLLER_PARAM_LIMITS        13
#define KITE_CONTROLLER_PARAM_LIMITS_E      5

// @pub 13;f;1;w;threshold;Distance in meters within which to throttle down to min limit
#define KITE_CONTROLLER_PARAM_THRESHOLD     14
#define KITE_CONTROLLER_PARAM_THRESHOLD_E   6

#define KITE_CONTROLLER_PARAM_ENTRIES       7

// subs
// @sub 20;21;f;1;target;Target heading (e.g. from Nav module)
#define KITE_CONTROLLER_SUB_TARGET          20
#define KITE_CONTROLLER_SUB_TARGET_ADDR     21
#define KITE_CONTROLLER_SUB_TARGET_E        0

// @sub 22;23;f;1;heading;Current heading (e.g. from Compass)
#define KITE_CONTROLLER_SUB_HEADING         22
#define KITE_CONTROLLER_SUB_HEADING_ADDR    23
#define KITE_CONTROLLER_SUB_HEADING_E       1

// @sub 24;25;f;1;distance;Distance to target, typically Nav.distance
#define KITE_CONTROLLER_SUB_DISTANCE       24
#define KITE_CONTROLLER_SUB_DISTANCE_ADDR  25
#define KITE_CONTROLLER_SUB_DISTANCE_E     2


#define KITE_CONTROLLER_SUBS               3

// modes
#define KITE_CONTROLLER_MODE_MANUAL        0
#define KITE_CONTROLLER_MODE_AUTOMATIC     1


static const char KITE_CONTROLLER_STR_KITE_CONTROLLER[] PROGMEM = "KiteController";

class KiteControllerModule:  public DroneModule {
protected:
  unsigned long _lastUpdate;
  float _iError;
  float _dError;
  float _lastError;
  float _lastHeading;
  uint8_t _lastMode;

public:

  KiteControllerModule(uint8_t id, DroneSystem* ds);
  
  void loop();
};

#endif
